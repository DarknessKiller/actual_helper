...
<div class="card bg-base-100 shadow-md" in:fade={{ duration: 400 }}>
  <div class="card-body">
    ...
    {#if status === 'error'}
      <div role="alert" class="alert alert-error mb-4" in:fly={{ y: -20, duration: 300 }}>
        <span>{errorMsg}</span>
        <button class="btn btn-sm btn-ghost" onclick={handleDismissError}>Dismiss</button>
      </div>
    {/if}
    ...
    <div class="form-control w-full mb-3">
      ...
      <div
        class="flex flex-col items-center justify-center w-full min-h-[120px] border-2 border-dashed rounded-lg p-6 cursor-pointer transition-all duration-200 {dragOver ? 'border-primary bg-primary/10' : 'border-base-300 bg-base-200 hover:border-primary hover:bg-base-200'}"
        ...
        {#if file}
          <div class="flex items-center gap-3 w-full min-w-0">
            ...
            <button
              ...
              onclick={(e) => { e.stopPropagation(); file = null; if (fileInput) fileInput.value = '' }}
              disabled={status === 'uploading'}
            >✕</button>
          </div>
        {:else}
